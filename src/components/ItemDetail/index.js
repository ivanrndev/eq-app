...
import {View, Text, StyleSheet, Dimensions, TextInput, TouchableOpacity, KeyboardAvoidingView, TouchableWithoutFeedback, Keyboard} from 'react-native';
import T from "../../i18n";
...
export const ItemDetail = (item) => {
  ...
  return (
    ...
    <View style={styles.body}>
      ...
      <TextInput
        style={{borderWidth: 1, width: Dimensions.get('window').width / 1.17, height: 35, marginLeft: 5, padding:5}}
        keyboardType='numeric'
        placeholder={T.t('set_quantity')}
        onChangeText={(num => setNumber(num))}
        defaultValue={currentQuantity[0]?.quantity}
      />
      {check && <Text style={{color: 'red', width: Dimensions.get('window').width / 1.05, margin: 5}}>
        Указанное значение превышает доступный остатоток, операция не может быть совершена</Text>}
      <View style={styles.buttonsContainer}>
        <TouchableOpacity onPress={() => saveButton()} style={{
          padding: 15,
          borderRadius: 10,
        ...
        <TouchableOpacity onPress={() => cancelButton()} style={{
          padding: 15,
          borderRadius: 10,
          borderWidth: 1,
          borderColor: '#2D2C71',
